...
(function () {
  ...
  // ---- Datas de check-in/check-out (input nativo YYYY-MM-DD -> DD-MM-YYYY) ----
  ...
  function init() {
    ...
    var panel = el("div", { class: "w4w-panel" }, ""
      ...
      + "</div>"
      + '<div class="w4w-field" data-field="checkin">'
      + "<label>Check-in</label>"
      + '<input type="date" name="checkin" />'
      + '<div class="w4w-error">Informe a data de check-in.</div>'
      + "</div>"
      + '<div class="w4w-field" data-field="checkout">'
      + "<label>Check-out</label>"
      + '<input type="date" name="checkout" />'
      + '<div class="w4w-error">Informe uma data de check-out depois do check-in.</div>'
      + "</div>"
      + '<div class="w4w-field" data-field="pessoas">'
      + "<label>Quantidade de pessoas</label>"
      + '<input type="number" name="pessoas" min="1" step="1" inputmode="numeric" />'
      + '<div class="w4w-error">Informe quantas pessoas (mínimo 1).</div>'
      + "</div>"
    ...
    var pessoasField = form.querySelector('[name="pessoas"]');
    var submitBtn = form.querySelector(".w4w-submit");

    checkinField.min = todayIso();
    checkoutField.min = addDaysIso(todayIso(), 1);

    checkinField.addEventListener("change", function () {
      if (!checkinField.value) return;
      checkoutField.min = addDaysIso(checkinField.value, 1);
    ...
    form.addEventListener("submit", function (e) {
      ...
      var checkinValue = checkinField.value;
      var checkoutValue = checkoutField.value;
      var pessoasValue = parseInt(pessoasField.value, 10);

      var nomeInvalid = nome.length < 2;
      var telefoneInvalid = telefoneDigits.length < 10;
      var checkinInvalid = !checkinValue;
      var checkoutInvalid = !checkoutValue || checkoutValue <= checkinValue;
      var pessoasInvalid = !pessoasField.value || isNaN(pessoasValue) || pessoasValue < 1;

      setInvalid(nomeField.closest(".w4w-field"), nomeInvalid);
      setInvalid(telefoneField.closest(".w4w-field"), telefoneInvalid);
      setInvalid(checkinField.closest(".w4w-field"), checkinInvalid);
      setInvalid(checkoutField.closest(".w4w-field"), checkoutInvalid);
      setInvalid(pessoasField.closest(".w4w-field"), pessoasInvalid);

      if (nomeInvalid || telefoneInvalid || checkinInvalid || checkoutInvalid || pessoasInvalid) return;
      ...
      Promise.all([
        ...
        var payload = {
          ...
          telefone: telefone,
          produto: produto,
          checkIn: isoToBrDate(checkinValue),
          checkOut: isoToBrDate(checkoutValue),
          qtdPessoas: pessoasValue,
