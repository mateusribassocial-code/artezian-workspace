# Condomínio — Moradas de Israel
> Taperapuã, Porto Seguro (BA). Dados compartilhados por todas as unidades do prédio.
> Preenchido em 2026-09-05. Atualizar quando mudar procedimento de portaria, horário de lazer ou plantão.

**Unidades que herdam este bloco:** FL10J — Apartamento do Reinaldo (Apto 04)

**Nome oficial no Google Maps:** Residencial Moradas de Israel — Tonziro
**Coordenadas:** `-16.3898988, -39.040346`
**Link:** https://maps.app.goo.gl/JUioonaZvquy4N9QA

> ⚠️ **Corrigir o `vizinhanca-imoveis.md` (linha 16):** ele registra o FL10J como *Residencial Village do Bosque*, e o Maps confirma **Moradas de Israel — Tonziro**. Nome errado no manual manda o hóspede pra portaria errada.

---

## Localização
| Dado | Valor | Fonte |
|---|---|---|
| Endereço | R. dos Ipês, 85 — Taperapuã | descrição FL10J |
| Cidade / UF / CEP | Porto Seguro / BA / 45810-000 | descrição FL10J |
| Praia mais próxima | Taperapuã — **392m em linha reta (~6 min a pé)** | medido via OSM, 05/09/2026 |
| Ponto de referência | Axé Moi, a 470m (~7 min a pé) | medido via OSM |
| Do aeroporto | **10 minutos de carro** | Mateus, 05/09/2026 |
| Centro histórico | ~8 km | descrição FL10J |
| Acesso à praia | ~6 min a pé. ⚠️ **A descrição comercial diz "acesso direto à praia" e a vizinhança diz 180m — os dois estão errados.** Medição por coordenada dá 392m até a linha de costa. Corrigir na descrição: hóspede que chega esperando sair do portão na areia reclama. | medido via OSM |

## Chegada e portaria
| Dado | Valor |
|---|---|
| Portaria 24h | Sim |
| Retirada da chave | **Na portaria** |
| Nome que vai na lista | **Nome do responsável pela reserva** |
| Exige documento com foto | **Sim, obrigatório** |
| Cofre / fechadura eletrônica | Não tem |
| Autorização prévia enviada pela Artezian | ⬜ PENDENTE — a portaria já recebe a lista de vocês, ou o hóspede chega e a portaria confere na hora? |

## Plantão Artezian
| Dado | Valor |
|---|---|
| WhatsApp de Reservas | +55 73 9937-3474 ⬜ **CONFERIR** — celular brasileiro tem 9 dígitos após o DDD, este veio com 8. Provável: (73) 99937-3474 |
| Horário de atendimento | ⬜ PENDENTE (padrão sugerido: 8h–22h, emergência 24h) |

## Área de lazer
| Facilidade | Existe | Horário / regra |
|---|---|---|
| Piscina adulto | Sim | ⬜ PENDENTE |
| Piscina infantil | Sim | ⬜ PENDENTE |
| Sauna | Sim | ⬜ PENDENTE |
| Churrasqueira coletiva | Sim | ⬜ PENDENTE (precisa reservar? tem carvão?) |

## Regras do condomínio
| Regra | Valor |
|---|---|
| Horário de silêncio | ⬜ PENDENTE |
| Visitante permitido | ⬜ PENDENTE |
| Visitante precisa autorização na portaria | ⬜ PENDENTE |
| Onde deixar o lixo | ⬜ PENDENTE |

## Região (herda de vizinhanca-imoveis.md — bloco TAPERAPUÃ)
Beach clubs, restaurantes e passeios já mapeados. **Falta levantar:** mercados e farmácias da região (Bloco C do checklist) — uma vez levantado, serve pra todas as unidades de Taperapuã.
